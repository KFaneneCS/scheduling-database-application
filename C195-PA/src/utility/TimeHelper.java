...
public class TimeHelper {
    ...
    private static boolean dateSpillover;
    /**
    ...
    /**
     * Method that returns boolean depending on whether there is spillover into
     * next day per user's default timezone.
     *
     * @return Returns true if 8am-10pm EST translates to midnight and later for user.
     */
    public static boolean hasDateSpillover() {
        return dateSpillover;
    }

    /**
     * Method that sets boolean depending on whether there is spillover into
     * next day per user's default timezone.
     *
     * @param spillover     Boolean that is true if time spills over beyond 11:59pm
     *                          per user's default time zone.
     */
    public static void setDateSpillover(boolean spillover) {
        dateSpillover = spillover;
    }
    ...
    public static boolean updateHasOverlap(int customerId, int appointmentId, ZonedDateTime checkStart, ZonedDateTime checkEnd) {
        ...
        for (Appointment appointment : AppointmentAccess.getAllAppointments()) {
            ...
            if (appointment.getCustomerId() == customerId) {
                ...
                if (isOverlapping(checkStart, checkEnd, start, end) && (appointment.getId() != appointmentId)) {
                    return true;
                }
